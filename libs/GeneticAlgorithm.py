...
class GeneticAlgorithm:
    ...
    def __init__(self,
                 ...
                 cpu_num          # Numero di cpu, per sfruttare uil calcolo parallelo dell'algoritmo genetico
                 ):
    ...
    def run(self, gen, pc=0.5, pm=0.05, debug=False):
        ...
        for g in range(gen):
            ...
            for k in range(len(new_population)):
                new_population[k] = self.mutation(new_population[k], pm, fitness)
            # Aggiornamento della popolazione
            self.population[:] = new_population[:]
            fitness = [self.fitness_fnc(self.population[i]) for i in range(len(self.population))]
            if debug:
                best_val, best_vec = self.fitness_bst2(fitness)
                fit_avg = sum(fitness) / len(fitness)
                np_covered = self.get_np_covered(self.population[best_vec])
                nr_covered = self.get_nr_covered(self.population[best_vec])
                subsets = sum(self.population[best_vec])
                self.times.append(g)
                ...
                if stall >= MAX_STALL:
                    return good_sol
                print('generation #', g, 'good_sol_val: ', best_val, 'fit_avg: ', fit_avg, 'best np_covered: ', np_covered, 'best nr_covered: ', nr_covered, 'subsets: ', subsets)
        return good_sol
