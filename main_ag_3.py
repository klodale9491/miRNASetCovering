...
def fitness3(solution):
    ...
    final_score = ((B1 * math.fabs(len(pos) - up_reg_cnt)) + (B2 * (len(neg)))) * math.pow(sum(solution), 3)
    return final_score
...
def save_solution(solution):
    file = open("results/metodo_prof/solutions.csv", "a")
    string = ""
    for bit in solution:
        string += str(bit)
    file.write(string + "\n")
...
def compute_mirna_frequency():
    with open("results/metodo_prof/solutions.csv", "r") as file:
...
def load_run_data():
    all_dataset = []
    for i in range(100):
        with open("results/metodo_prof/runs/run_"+str(i)+".csv") as file:
...
if __name__ == "__main__":
    ...
    '''
    for i in range(100):
        GA = GeneticAlgorithm(fitness3, fitness_cmp_min, 0, 1, 100, len(mirna_data), cpu_num=2)
        GA.set_params(mirna_data, up_reg_cnt)
        sol = GA.run(gen=1500, pc=0.8, pm=1/len(mirna_data), debug=True)
        GA.save_data(i, "results/metodo_prof/runs/")
        save_solution(sol)
    '''